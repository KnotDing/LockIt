        ...
        return Menu(title) {
            ...
            ForEach(stride(from: -30, through: -90, by: -5).map { $0 }, id: \.self) { value in
                ...
                if value > settings.weakSignalThreshold {
                    Button(String(format: NSLocalizedString("DBM_VALUE_FORMAT", comment: "Format for dBm value"), value)) {
                        settings.screenOnSignalThreshold = value
                        BluetoothManager.shared.setup(settings: settings)
                    }
                }
            }
    ...
    var body: some View {
        Menu(String(format: NSLocalizedString("WEAK_SIGNAL_TIMEOUT_MENU_TITLE", comment: "Menu title for weak signal timeout"), Int(settings.weakSignalTimeout))) {
            ForEach([5, 10, 20, 30, 60], id: \.self) { value in
                Button(String(format: NSLocalizedString("SECONDS_VALUE_FORMAT", comment: "Format for seconds value"), value)) {
                    settings.weakSignalTimeout = TimeInterval(value)
                    BluetoothManager.shared.setup(settings: settings)
                }
            }
        }
    ...
    var body: some View {
        Menu(String(format: NSLocalizedString("DISCONNECT_TIMEOUT_MENU_TITLE", comment: "Menu title for disconnect timeout"), Int(settings.disconnectTimeout))) {
            ForEach([5, 10, 20, 30], id: \.self) { value in
                Button(String(format: NSLocalizedString("SECONDS_VALUE_FORMAT", comment: "Format for seconds value"), value)) {
                    settings.disconnectTimeout = TimeInterval(value)
                    BluetoothManager.shared.setup(settings: settings)
                }
            }
        }
...
struct MenuBarContentView: View {
    ...
    @State private var peripheralsSnapshot: [DiscoveredDevice] = []
    ...
    var body: some View {
        Group {
            ...
            Divider()
            Menu {
                SelectBluetoothDeviceMenu(settings: settings, peripherals: peripheralsSnapshot)
            } label: {
    ...
    private func setupTimer() {
        invalidateTimer()
        // 立即更新一次数据
        self.peripheralsSnapshot = BluetoothManager.shared.discoveredPeripherals
        self.updateTimer = Timer.scheduledTimer(withTimeInterval: 30.0, repeats: true) { _ in
            #if DEBUG
            print("⏰ Timer fired: Updating peripherals snapshot.")
            #endif
            self.peripheralsSnapshot = BluetoothManager.shared.discoveredPeripherals
        }
    }
...
struct LockItApp: App {
    ...
    init() {
        ...
        #if DEBUG
        print("LockItApp: Setup BluetoothManager with \(settings.selectedPeripheralUUID ?? "Nil").")
        #endif
        BluetoothManager.shared.setup(settings: settings)
    }
    var body: some Scene {
